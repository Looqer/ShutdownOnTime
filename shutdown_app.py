...
def execute_shutdown(shutdown_time):
    current_time_execute = time.strftime("%H:%M:%S")
    while current_time_execute < shutdown_time:
        time.sleep(1)
        current_time_execute = time.strftime("%H:%M")
...
def update_time():
    current_time_update = time.strftime("%H:%M:%S")
...
def update_remaining_time():
    ...
    remaining_time = (shutdown_time - (current_time_remain - start_time)).total_seconds()
...
def start_shutdown_hour_thread():
    ...
    shutdown_time = datetime.strptime(get_shutdown_time(), "%H:%M")
    start_time = datetime.now()
    thread = Thread(target=execute_shutdown, args=(shutdown_time.strftime("%H:%M"),))
    thread.start()
